...
class Tmahasiswa(db.Model, UserMixin): #tambahkan
    ...
    pendataans=  db.relationship('Tpendataan', backref='mahasiswas', lazy=True)
...
class Tpendataan(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    kategori_surat = db.Column(db.String(50), nullable=False)
    surat = db.Column(db.String(50), nullable=False)
    detail_surat = db.Column(db.String(300), nullable=False) 
    tgl_surat = db.Column(db.DateTime(100), nullable=False, default=datetime.utcnow) 
    mahasiswa_id = db.Column(db.Integer, db.ForeignKey('tmahasiswa.id'))

    def __repr__(self):
        return f"Tpendataan('{self.kategori_surat}','{self.surat}','{self.detail_surat}','{self.tgl_surat}')"
